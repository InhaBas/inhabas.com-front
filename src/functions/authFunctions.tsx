...
export const GetRoleAuthorization = () => {
  ...
  const isAuthorizedOverSecretary = ['CHIEF', 'VICE_CHIEF', 'EXECUTIVES', 'SECRETARY'].includes(
    role,
  );
  const isAuthorizedOverBasic = [
    'CHIEF',
    'VICE_CHIEF',
    'EXECUTIVES',
    'SECRETARY',
  ...
  const isAccessible = (authorization: AuthorizationInterface['authorization']) => {
    ...
    if (
      authorizationLocation[authorization]?.includes(location?.pathname) &&
